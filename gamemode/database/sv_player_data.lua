...
local function IsBannedCallback(data, ply)
...
local function CheckCallback(data, ply)
    if data[1] then
        TK.DB:UpdateQuery("server_player_record",
        {
            ip = ply.ip,
            steam_name = ply:GetName()
        },
        {
            ["steamid = %s"] = ply.steamid
        })

        TK.DB:SetNetworkData(ply, data[1])
        TK.DB:LoadPlayer(ply)
    else
        TK.DB:InsertQuery("server_player_record",
        {
            steamid = ply.steamid,
            ip = ply.ip,
            uniqueid = ply.uid,
            steam_name = ply:GetName(),
            nick_name = ply:GetSafeName()
        })

        TK.DB:SetNetworkData(ply,
        {
            nick_name = ply:GetSafeName(),
            rank = 1,
            team = 1
...
function TK.DB:CheckForData(ply)
    self:SelectQuery("server_player_record", nil,
    {
        ["steamid = %s"] = ply:SteamID()
    },
    nil, 1, CheckCallback, ply)
end
...
local function LoadCallback(data, ply, dbtable)
    if !data[1] then
...
function TK.DB:LoadPlayer(ply)
    for dbtable, data in pairs(list.Get("TK_Database")) do
        ...
        self:SelectQuery(dbtable, nil,
        {
            ["steamid = %s"] = ply.steamid
        }, nil, 1, LoadCallback, ply, dbtable)
    end
end

function TK.DB:SetPlayerData(ply, dbtable, data)
    if dbtable == nil then print("nil dbtable!") end
    if !self.PlayerData[ply.uid] then print("Returning..(ply.uid)") return end
    if !self.PlayerData[ply.uid][dbtable] then print("No data for table") return end
    
    local p_data = self.PlayerData[ply.uid][dbtable]
    local n_data = {}

    for idx, val in pairs(data or {}) do
        if !p_data[idx] or p_data == val then
            continue
        end
        p_data[idx] = val
...
function TK.DB:UnloadPlayer(ply)
    ...
    for dbtable, data in pairs(self.PlayerCache[ply.uid]) do
        ...
        print(table.ToString(n_data, "n_data", true))
...
hook.Add("PlayerAuthed", "TK_Load_Player", function(ply)
    ...
    TK.DB:SelectQuery("server_ban_data", {"idx"}, {
        ["ban_lifted = 0"] = 0,
        ["(ban_lenght = 0 OR (ban_start + ban_lenght) > UNIX_TIMESTAMP())"] = 0,
        ["ply_steamid = %s"] = ply.steamid
    }, nil, 1, IsBannedCallback, ply)
end)

hook.Add("PlayerDisconnected", "TK_Unload_Player", function(ply)
    timer.Remove("TK_Data_Cache_" .. ply.uid)
    TK.DB:UnloadPlayer(ply)
