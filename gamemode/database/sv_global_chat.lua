...
hook.Add("InitPostEntity", "TKDB_GChat", function()
    TK.DB:SelectQuery("server_globalchat", {"msg_idx"}, {
        ["msg_idx > %s"] = 0
    }, {"msg_idx",  "DESC"}, 1, function(data)
        if !data[1] then
            TK.DB.last_gmsg = 0
    ...
    timer.Create("TKDB_GChat", 10, 0, function()
        ...
        TK.DB:SelectQuery("server_globalchat", nil, {
            ["msg_idx > %s"] = TK.DB.last_gmsg
        }, {"msg_idx"}, nil, function(data)
